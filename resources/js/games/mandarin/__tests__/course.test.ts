...
describe('canonical course data', () => {
  it('parses the shipped JSON with the expected shape', () => {
    ...
    expect(course.targetById.size).toBe(30)
    expect(course.supportById.size).toBe(18)
    expect(course.utteranceById.size).toBe(50)
    expect(course.exerciseById.size).toBe(50)
    expect(course.constructionById.size).toBe(5)
    expect(course.checkpointById.size).toBe(10)
  })
  ...
  it('keeps the ten reserved utterances out of every node and scene', () => {
    const course = loadCourse()
    expect(course.reservedUtteranceIds.size).toBe(10)
  ...
  it('unlocks the checkpoint only after the last node', () => {
    const course = loadCourse()
    expect(course.course.checkpointPolicy.unlockAfterNodeId).toBe('s5n2')
    expect(course.nodes.at(-1)?.id).toBe('s5n2')
  })
