...
function bootstrapBody(signedIn: boolean): unknown {
  return {
    ...
    audio: { courseId: 'mandarin-foundations', contentVersion: '1.0.0', results: [] },
...
const event: PracticeEvent = {
  courseId: 'mandarin-foundations', contentVersion: '1.0.0', schemaVersion: 1, clientEventId: 'e1', clientInstanceId: 'c', sessionId: 's',
...
describe('HttpMandarinGateway', () => {
  ...
  it('derives due targets from the server review log with the pinned scheduler', async () => {
    const { gateway, calls } = gatewayWith((call) => call.url === MANDARIN_API.bootstrap
      ...
      : jsonResponse(200, {
        courseId: 'mandarin-foundations', contentVersion: '1.0.0', lastSequence: 2, completedNodeIds: ['s1n1'], completedSceneIds: [], currentNodeId: 's1n2',
  ...
  it('passes audio resolutions through unchanged, treats auth failures as sign_in_required, and bounds batches', async () => {
    ...
    const { gateway } = gatewayWith((call) => call.url === MANDARIN_API.resolve
      ? (status === 202 ? jsonResponse(202, { courseId: 'mandarin-foundations', contentVersion: '1.0.0', results: [ready] }) : jsonResponse(status, {}))
      : jsonResponse(200, ready))
    const request = { courseId: 'mandarin-foundations', contentVersion: '1.0.0', sources: [{ sourceKind: 'utterance' as const, sourceId: '01a', variant: 'normal' as const }] }
